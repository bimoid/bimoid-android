...
//                System.out.println("cliAuthRevoke: userId="+userId);
    ...
    public static void reqAvatar(OBIMPConnection con, byte[] ava_hash) {
        if(con != null) {
            try {
                System.out.println("ava_hash: " + ava_hash);
                Packet ava_req = new Packet(PacketHandler.OBIMP_BEX_UA, PacketHandler.OBIMP_BEX_UA_CLI_AVATAR_REQ); //OBIMP_BEX_UA_CLI_AVATAR_REQ
                ava_req.append(new wTLD(0x0001, new OctaWord(ava_hash)));
                con.out.write(ava_req.asByteArray(con.getSeq()));
                con.out.flush();
            } catch(Exception ex) {
                System.out.println("Error:\n");
                ex.printStackTrace();
